...
from zipfile import BadZipFile, LargeZipFile, \
    _EndRecData, _check_zipfile, _ZipDecrypter, compressor_names, \
    _get_compressor, ZipExtFile as _ZipExtFile
...
if sys.version_info < (3, 6):
    ...
    os_PathLike = (pathlib.Path, pathlib.PosixPath)
    ...
    from zipfile import ZipInfo, is_zipfile, _check_compression, _Tellable, \
        _SharedFile, ZipExtFile
...
class _ZipWriteFile(io.BufferedIOBase):
    def __init__(self, zf, zinfo, zip64, pwd):
        self._zinfo = zinfo
        self._zip64 = zip64
        self._zipfile = zf
        ...
        if pwd:
            ...
            self._encrypt = lambda x: bytes(map(ze, x))
            check_byte = (zinfo._raw_time >> 8) & 0xff
            encryption_header = randomfunc(11) + struct.pack("B", check_byte)
            zf.fp.write(self._encrypt(encryption_header))
        else:
    ...
    def write(self, data):
        ...
        self._fileobj.write(self._encrypt(data))
        return nbytes
    ...
    def close(self):
        ...
        if self._compressor:
            ...
            self._fileobj.write(self._encrypt(buf))
...
class ZipFile:
    ...
    def open(self, name, mode="r", pwd=None, *, force_zip64=False):
        ...
        if mode == 'w':
            return self._open_to_write(zinfo, force_zip64=force_zip64, pwd=pwd or self.pwd)
    ...
    def _open_to_write(self, zinfo, force_zip64=False, pwd=None):
        ...
        if pwd:
            zinfo.flag_bits |= 0x08 | 0x01  # set stream and encrypted
            zinfo._raw_time = (
        ...
        self._writing = True
        return _ZipWriteFile(self, zinfo, zip64, pwd)
    ...
    def write(self, filename, arcname=None, compress_type=None, pwd=None):
        ...
        if zinfo.is_dir():
            ...
            with open(filename, "rb") as src, self.open(zinfo, 'w', pwd=pwd) as dest:
    ...
    def writestr(self, zinfo_or_arcname, data, compress_type=None, pwd=None):
        ...
        with self._lock:
            with self.open(zinfo, mode='w', pwd=pwd) as dest:
                dest.write(data)
